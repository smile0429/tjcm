...
(function(root, factory) {
    ...
    jedfn.getValue = function (valobj) {
        ...
        if (valobj == undefined && jet.isBool(opts.isShow)){
            ...
            var isValShow = jet.isBool(opts.isShow) ? (that.getValue() == "") : !jet.isBool(opts.isShow),
                ...
                defObj = [{ YYYY:dateY,MM:dateM,DD:dateD, hh:timeh,mm:timem,ss:times,zz:window.parseInt('00')},
                    { YYYY:dateY,MM:dateM,DD:dateD, hh:timeh,mm:timem,ss:times,zz:window.parseInt('00')}];
            if (isValShow) {
                //目标为空值则获取当前日期时间
    ...
    jedfn.renderHtml = function(ys, ms, ds, opts,boxcls){
        ...
        if (testhh) {
            ...
            if (isrange) {
                if (that.getValue() == "") {
                    hms = mlens == 7 ? rehms.concat(rehms) : rehms.concat([window.parseInt('00'), window.parseInt('00'), window.parseInt('00')]);
                } else {
    ...
    jedfn.eachYM = function(y, m,opts,boxCell,clsCell) {
        ...
        $.each(new Array(multiPane ? 1 : 2),function (s) {
            var retSetCls = function (sym,gym,eym) {
                var sval = sym.replace(gr,""), gval = gym.replace(gr,""), evalue = eym.replace(gr,"");
                if (/YYYY-MM-DD/g.test(jet.isparmat(that.format))){
                    ...
                    if (parseInt(sval) == parseInt(gval)) {
                        ...
                        return "actdate";
                    } else if (parseInt(sval) > parseInt(gval) && parseInt(sval) < parseInt(evalue)) {
                        return "contain";
                    } else if (parseInt(sval) == parseInt(evalue)) {
                        if (!testhh) {
                            that.areaVal.push(sym);
                            that.areaStart = true;
                        }
        ...
        var contd = ymscon.find("td"),ymstit = that.maincon(".ymsbox",0),
            ...
            sval = contd.eq(0).text(), evalue = contd.eq(eqNum).text();
        var mnx = [(formatYY ? sval:sval.substring(0,4)),(formatYY ? evalue:evalue.substring(0,4))];
        ymstit.find("p").remove();
        ymstit.append("<p>"+sval+" ~ "+evalue+"</p>").attr({min:mnx[0],max:mnx[1]});
    };
    //初始验证正则
    ...
    jedfn.clickDays = function (opts,boxCell) {
        ...
        if(opts.festival && lang.name == "cn") {
            ...
            tdCls.on("mouseover", function () {
                ...
                tipDiv.css({"z-index":  (opts.zIndex == undefined ? 100000 + 5 : opts.zIndex + 5),top:tipPos.top,left:tipPos.left,position:"absolute",display:"block"});
    ...
    $.splitDate = function (str) {
        ...
        return {
            YYYY:parseInt(sdate[0]),MM:parseInt(sdate[1])||window.parseInt('00'),DD:parseInt(sdate[2])||window.parseInt('00'),
            hh:parseInt(sdate[3])||window.parseInt('00'),mm:parseInt(sdate[4])||window.parseInt('00'),ss:parseInt(sdate[5])||window.parseInt('00')
        };
    };
    //获取年月日星期
